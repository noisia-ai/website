...
const makeNoisePattern = (ctx: CanvasRenderingContext2D) => {
  ...
  for (let i = 0; i < image.data.length; i += 4) {
    ...
    image.data[i + 3] = Math.floor(randomBetween(18, 42));
  }
...
export const initFluidBackground = () => {
  const canvas = document.querySelector<HTMLCanvasElement>("[data-fluid-background]");
  if (!canvas) return;
